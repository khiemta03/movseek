...
const ReviewsAndRating: React.FC<ReviewsAndRatingProps> = ({ tv, isSignedIn, user_id, avatar, username }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const commentsResponse = await getCommentsByMedia(parseInt(tv.id), 'tv_show');
        ...
        const ratingsResponse = await getRatingsByMedia(parseInt(tv.id), 'tv_show');
    ...
    if (isSignedIn) {
      try {
        ...
        if (myComment.trim() != originReview.trim()) {
          ...
          if (myComment.trim() !== '') {
            if (isCommented) {
              await updateComment(avatar, myComment, parseInt(tv.id), 'tv_show', user_id, username);
              commentToast = 'Your comment has been updated.';
            } else {
              await addComment(avatar, myComment, parseInt(tv.id), 'tv_show', user_id, username);
              commentToast = 'Your comment has been added.';
              setIsCommented(true);
            }
          } else if (isCommented) {
            await deleteComment(user_id, parseInt(tv.id), 'tv_show');
        ...
        if (myRating != originRating) {
          ...
          if (myRating != null) {
            if (isRated) {
              await updateRating(avatar, myRating / 10, parseInt(tv.id), 'tv_show', user_id, username);
              ratingToast = 'Your rating has been updated.';
            } else {
              await addRating(avatar, parseInt(tv.id), myRating / 10, 'tv_show', user_id, username);
              ratingToast = 'Your rating has been added.';
              setIsRated(true);
            }
          } else if (isRated) {
            await deleteRating(user_id, parseInt(tv.id), 'tv_show');
      ...
      <div className="my-2">
        <div className="w-full mx-auto p-4 border rounded-lg shadow-lg bg-white space-y-6">
          <div>
            ...
            <div className="space-y-2">
              <h2 className="text-lg font-semibold">{`${isRated ? 'My rating' : 'Rate the tv show'}`}</h2>
